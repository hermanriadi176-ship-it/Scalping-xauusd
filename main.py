...
config = {
    "TELEGRAM_BOT_TOKEN": os.environ.get("8488792485:AAHQ35Pu1gdUtyGqP5k8FiWme-7ICqNNdWo", ""),
    "TELEGRAM_CHAT_ID": os.environ.get("206050507", ""),
    "TWELVE_DATA_API_KEY": os.environ.get("5e272bcdc25544dda331772e51a01ea2", ""),
    "TIMEFRAME": "5min",
    "RISK_REWARD": "1:2",
    "LOT_SIZE": 0.1,
    "IS_RUNNING": False
...
<body>
    <div class="container">
        ...
        <div class="card-group">
            <div class="card"><h3>STATUS BOT</h3><p class="{% if running %}status-running{% else %}status-stopped{% endif %}">{{ "RUNNING" if running else "STOPPED" }}</p></div>
            <div class="card"><h3>TOTAL SINYAL</h3><p>{{ stats.total_signals }}</p></div>
            <div class="card"><h3>WIN RATE</h3><p>{{ stats.win_rate }}</p></div>
            <div class="card"><h3>TOTAL PNL</h3><p>{{ stats.total_pnl }}</p></div>
        </div>
        <form method="POST" action="/update">
            <h3>Pengaturan API & Bot</h3>
            <label>Telegram Bot Token:</label><input type="text" name="telegram_token" value="{{ config.TELEGRAM_BOT_TOKEN }}">
            <label>Telegram Chat ID:</label><input type="text" name="telegram_chat_id" value="{{ config.TELEGRAM_CHAT_ID }}">
            <label>Twelve Data API Key:</label><input type="text" name="twelve_data_key" value="{{ config.TWELVE_DATA_API_KEY }}">
            ...
            <div style="display: flex; gap: 10px;">
                ...
                {% if running %}<button type="submit" name="action" value="stop" style="background: #e74c3c; color: #fff;">Stop Bot</button>{% else %}<button type="submit" name="action" value="start" style="background: #2ecc71; color: #fff;">Start Bot</button>{% endif %}
            </div>
        </form>
        <h3>Riwayat Sinyal Aktif (Pengecekan Terakhir: {{ stats.last_check }})</h3>
        <table>
            <tr><th>Waktu</th><th>TF</th><th>Tipe</th><th>Entry</th><th>SL</th><th>TP</th></tr>
            {% for sig in stats.active_signals %}
            <tr><td>{{ sig.time }}</td><td>{{ sig.tf }}</td><td style="color: {% if sig.type == 'BUY' %}#2ecc71{% else %}#e74c3c{% endif %}; font-weight: bold;">{{ sig.type }}</td><td>{{ sig.entry }}</td><td>{{ sig.sl }}</td><td>{{ sig.tp }}</td></tr>
            {% else %}
            <tr><td colspan="6" style="text-align: center; color: #777;">Belum ada sinyal aktif. Silakan Start Bot.</td></tr>
            {% endfor %}
        </table>
    </div>
</body>
...
def update():
    ...
    if action == "start": config["IS_RUNNING"] = True
    elif action == "stop": config["IS_RUNNING"] = False
